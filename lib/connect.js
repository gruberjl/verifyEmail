...
exports.isEmailValid = (connection, obj) => new Promise((res, rej) => {
  ...
  const onResponse = (data) => {
    ...
    if (isResponseOk(message)) {
      obj.statusCode = 200
      res({isValid: true, message, connection})
    } else {
      obj.statusCode = 500
      res({isValid: false, message, connection})
    }
  }
...
exports.connect = (mxRecords) => p.untilSuccess(mxRecords, exports.newConnection)

exports.verifyEmails = (mxRecords, group) => exports.connect(mxRecords)
  .then((connection) =>
    p.waterfall(mxRecords, (record) => exports.isEmailValid(connection, obj.email))
      .then((results) => ({connection, results})))
